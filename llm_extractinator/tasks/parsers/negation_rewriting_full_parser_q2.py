...
class OutputParser(BaseModel):
    negation_types_q2: List[NegationType] = Field(default_factory=list, description="List of detected negation types in the text")
    is_negated_q2: bool = Field(..., description="Whether negation occurs in the text based on the taxonomy")
    remarks_q2: str = Field("", description="remarks or comments about the negation detection")
    rewritten_query_q2: str = Field(..., description="Rewritten positive query or query with exclusion criteria removed if rewriting is not possible due to information loss")
    exclusion_criteria_q2: List[str] = Field([], description="List of terms or phrases to exclude from results")
    explanation_q2: str = Field("", description="Brief explanation of rewrite or filtering decision")
